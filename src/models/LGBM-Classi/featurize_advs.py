...
train_output = os.path.join(sys.argv[2], 'cbasic_train.csv')
val_output = os.path.join(sys.argv[2], 'cbasic_validation.csv')
test_output = os.path.join(sys.argv[2], 'cbasic_test.csv')

fs_select = params['fs_select']

col_mask = ['Age', 'Ethnicity', 'Educational_Level',
...
enc_cols = ['Ethnicity', 'Educational_Level', 'Income', 'Deposit_type']
cat_cols = ['Meal_Type', 'Visted_Previously', 'Previous_Cancellations', 'Required_Car_Parking','Use_Promotion']
...
def main():
    ...
    std_tr_df = tp_tr_df.copy()
    std_val_df = tp_val_df.copy()
    std_ts_df = tp_ts_df.copy()

    std_tr_df['Age'] = scaler.fit_transform(tp_tr_df['Age'].values.reshape(-1,1))
    std_val_df['Age'] = scaler.transform(tp_val_df['Age'].values.reshape(-1,1))
    std_ts_df['Age'] = scaler.transform(tp_ts_df['Age'].values.reshape(-1,1))
    std_tr_df['Room_Rate'] = scaler.fit_transform(tp_tr_df['Room_Rate'].values.reshape(-1,1))
    std_val_df['Room_Rate'] = scaler.transform(tp_val_df['Room_Rate'].values.reshape(-1,1))
    std_ts_df['Room_Rate'] = scaler.transform(tp_ts_df['Room_Rate'].values.reshape(-1,1))

    enc_tr_df,maps = cat_to_int(std_tr_df, ['Reservation_Status']+cat_cols, {'Reservation_Status' : {'Check-In' : 1, 'Canceled' : 2, 'No-Show': 3},})
    enc_val_df = cat_to_int(std_val_df, ['Reservation_Status']+cat_cols, maps)
    enc_ts_df = cat_to_int(std_ts_df, cat_cols, maps)
    ...
    db_ts_df = woe_enc(enc_ts_df, wr_woe_enc, enc_cols)

    X_tr, y_tr = smote.fit_resample(db_tr_df.loc[:, col_mask[:-1]], db_tr_df.loc[:,'Reservation_Status'])
    ntr_df = X_tr.join(y_tr)

    ms_tr_df = ntr_df.loc[:, col_mask]
